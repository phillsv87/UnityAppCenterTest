...
public class SpotiftyApi : MonoBehaviour
{
    ...
    private async Task SignInAsync(CancellationToken cancel)
    {
        ...
        var r=await client.GetAsync(ApiBaseUrl+"Spotify/State",cancel);
    ...
    {
        var r = await client.GetAsync(ApiBaseUrl + "Spotify/Token?state=" + Uri.EscapeDataString(state),cancel);
        r.EnsureSuccessStatusCode();
